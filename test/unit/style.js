...
describe('Style', function () {
    ...
    describe('applyToCanvasPolygon', () => {
        it('with fill.pattern = img', function (done) {
            ...
            style1.applyToCanvasPolygon(txtCtx, matrix, invCtxScale, polygon)
                .then(() => {
                    assert.equal(txtCtx.fillStyle.constructor.name, 'CanvasPattern');
                    assert.equal(txtCtx.globalAlpha, style1.fill.opacity);
                    done();
                }).catch(done);
        });
        it('with fill.color = #0500fd', function (done) {
            ...
            style1.applyToCanvasPolygon(txtCtx, matrix, invCtxScale, polygon)
                .then(() => {
                    assert.equal(txtCtx.fillStyle, '#0500fd');
                    assert.equal(txtCtx.globalAlpha, style1.fill.opacity);
                    done();
                }).catch(done);
        });
