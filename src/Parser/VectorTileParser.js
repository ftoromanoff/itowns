...
function vtFeatureToFeatureGeometry(vtFeature, feature, classify = false) {
    ...
    while (pbf.pos < end) {
        ...
        if (cmd === 1 || cmd === 2) {
            ...
            count++;
            const coordIn = project(
                x,
                y,
                vtFeature.tileNumbers,
                vtFeature.extent);
            geometry.pushCoordinatesValues(feature, x, y, undefined, coordIn);
            if (count == 1) {
                firstPoint.set(x, y);
                firstPoint.coordIn = coordIn;
                lastPoint.set(x, y);
            ...
            if (count) {
                count++;
                geometry.pushCoordinatesValues(feature, firstPoint.x, firstPoint.y, undefined, firstPoint.coordIn);
                if (isPolygon) {
